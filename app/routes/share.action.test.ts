...
describe("share/ action", () => {
  ...
  test("should return redirect on success", async () => {
    const response = await action(
      getActionParams(JSON.stringify({ content: validContent, expiry: ShareExpiry.TOMORROW })),
    );
  ...
  test("should 400 on invalid expiry", async () => {
    const response = await catchResponse(
      action(getActionParams(JSON.stringify({ content: validContent, expiry: ShareExpiry.NEVER }))),
    );
    expect(response.status).toBe(400);
  });
